...
/**
 * Create a styled LVGL table and place it on a parent.
 *
 * @param parent     Parent LVGL object (must be valid).
 * @param style      Style to apply (must be lv_style_init()'d by the caller and outlive the table).
 *                   If nullptr, only object-level paddings are set.
 * @param font       Font to use for cell text (can be nullptr to keep default).
 * @param align      Alignment relative to parent (e.g., LV_ALIGN_TOP_LEFT).
 * @param pos_x      X offset from the alignment point.
 * @param pos_y      Y offset from the alignment point.
 * @param width      Table width.
 * @param height     Table height.
 * @param pad_left   Left padding for table items (cells).
 * @param pad_ver    Vertical padding for table items (cells).
 *
