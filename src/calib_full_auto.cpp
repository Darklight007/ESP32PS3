...
namespace
{
    ...
    static void full_auto_timer_cb(lv_timer_t *t)
    {
        ...
        case FullAutoPhase::INL:
            ...
            log_set_title("6/6 ADC INL Calibration");
            log_reset();
            log_clear();
            // Show INL window without calling LoadCalibrationData() (would overwrite CalBank)
            open_inl_window_no_load();
    ...
    {
        Serial.println("[FullAuto] CB entered");
        if (lv_event_get_code(e) != LV_EVENT_VALUE_CHANGED)
            return;

        Serial.println("[FullAuto] VALUE_CHANGED");
        lv_obj_t *obj = lv_event_get_current_target(e);
        const char *btn_txt = lv_msgbox_get_active_btn_text(obj);
        Serial.printf("[FullAuto] btn_txt='%s'\n", btn_txt ? btn_txt : "NULL");
        bool is_ok = (btn_txt && strcmp(btn_txt, "OK") == 0);

        Serial.println("[FullAuto] calling msgbox_close_deferred");
        msgbox_close_deferred(obj);
        Serial.println("[FullAuto] msgbox_close_deferred returned");
        ...
        // Create log window
        Serial.println("[FullAuto] Creating log window");
        create_log_window("Full Auto Calibration");
        log_reset();
        log_clear();
        Serial.println("[FullAuto] Log window created");
        ...
        if (!auto_timer)
        {
            Serial.println("[FullAuto] ERROR: Failed to create auto_timer (LVGL heap full)");
            g_calibration_in_progress = false;
            g_log_window_keep_open = false;
            phase = FullAutoPhase::IDLE;
            return;
        }
        Serial.println("[FullAuto] Timer created, calibration started");
    }
