...
namespace {
    ...
    static void slider_adcAVG_event_cb(lv_event_t* e) {
        ...
        char buf[6]; lv_snprintf(buf, sizeof(buf), "%-d", (uint16_t)std::pow(2, v));
        lv_label_set_text(lv_obj_get_child(lv_obj_get_parent(slider), 1), buf);
    ...
    static void slider_decimalPoints_event_cb(lv_event_t* e) {
        auto* slider = lv_event_get_target(e);
        uint16_t v = lv_slider_get_value(slider);
        char buf[4]; lv_snprintf(buf, sizeof(buf), "%i", v);
        lv_label_set_text(lv_obj_get_child(lv_obj_get_parent(slider), 1), buf);

        switch (v) {
            case 1: PowerSupply.Voltage.restrict = "%+05.1f";
                    PowerSupply.Current.restrict = "%+05.1f";
                    PowerSupply.Power  .restrict = "%+05.1f"; break;
            case 2: PowerSupply.Voltage.restrict = "%+06.2f";
                    PowerSupply.Current.restrict = "%+06.2f";
                    PowerSupply.Power  .restrict = "%+06.2f"; break;
            case 3: PowerSupply.Voltage.restrict = "%+07.3f";
                    PowerSupply.Current.restrict = "%+07.3f";
                    PowerSupply.Power  .restrict = "%+07.3f"; break;
            case 4: PowerSupply.Voltage.restrict = "%+08.4f";
                    PowerSupply.Current.restrict = "%+08.4f";
                    PowerSupply.Power  .restrict = "%+08.4f"; break;
            default:
    ...
    static void build_adc_calibration_window(lv_obj_t** win_holder,
                                             ...
                                             setting_GUI& gui,
                                             const CalPrefill& pf) {
        if (*win_holder) { lv_obj_clear_flag(*win_holder, LV_OBJ_FLAG_HIDDEN); return; }
        ...
        *win_holder = win;
        ...
        static lv_style_t style_btn_loadSave; static bool sbtn{};
        if (!sbtn) { lv_style_init(&style_btn_loadSave);
                     lv_style_set_bg_color(&style_btn_loadSave, lv_palette_main(LV_PALETTE_BLUE)); sbtn = true; }

        static lv_style_t style_lbl, style_val; static bool sstyles{};
        if (!sstyles) {
        ...
        lv_point_t offset{10,20}; lv_coord_t w = 110, col = w + 30, row = 38; lv_point_t pad{5,5};
    ...
    static lv_obj_t* create_pushbutton(lv_obj_t* parent, const char* /*icon*/,
        ...
        if (event_cb) lv_obj_add_event_cb(btn, event_cb, LV_EVENT_RELEASED, nullptr);
        return obj;
    }
} // namespace
...
void SettingMenu(lv_obj_t* parent) {
    ...
    create_pushbutton(section, nullptr, [](lv_event_t* e){ // DAC window open (private)
        if (win_DAC_calibration) { lv_obj_clear_flag(win_DAC_calibration, LV_OBJ_FLAG_HIDDEN); return; }
    ...
    char version_text[48]; snprintf(version_text, sizeof(version_text), "Version %s", SOFTWARE_VERSION);
