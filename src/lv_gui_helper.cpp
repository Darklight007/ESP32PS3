...
LVButton::LVButton(lv_obj_t* parent,
    ...
    if (style) lv_obj_add_style(btn_, style, LV_PART_MAIN);
    if (callback) lv_obj_add_event_cb(btn_, callback, LV_EVENT_RELEASED, nullptr);
}
...
lv_obj_t* LVLabel::create(lv_obj_t* parent, const char* labelText, const lv_obj_t* base,
    ...
    if (base) lv_obj_align_to(label, base, LV_ALIGN_OUT_BOTTOM_LEFT, x_ofs, y_ofs);
    if (style) lv_obj_add_style(label, style, LV_PART_MAIN);
    return label;
}
...
lv_obj_t* LVLine2::create(lv_obj_t* parent,
    ...
    if (!parent || !style || !points) return nullptr;
...
lv_obj_t* LVLine::create(lv_obj_t* parent,
                         ...
                         lv_coord_t x_ofs,
                         lv_coord_t y_ofs) {
    if (!parent || !style || points.size() != 2) return nullptr;
