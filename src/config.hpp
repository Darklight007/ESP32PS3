...
#include <Arduino.h>
#include <lvgl.h>         // for lv_* types used below
#include <TFT_eSPI.h>
#include <Keypad.h>
...
// Forward decls (avoid heavy headers here)
class TFT_eSPI;
class Keypad_MC17;
class FFTHandler;
class Device;
...
// ===== Screen =====
extern uint16_t screenWidth;    // = 320 (in .cpp)
extern uint16_t screenHeight;   // = 240 (in .cpp)

extern lv_disp_t*        lv_disp;     // LVGL display handle
extern lv_disp_draw_buf_t draw_buf;   // LVGL draw buffer descriptor
...
// Backlight PWM (compile-time constants are safe in headers)
inline constexpr uint8_t lcdBacklightPin    = 26;
inline constexpr int     freq               = 5000;
inline constexpr uint8_t lcdBacklightChannel= 1;
inline constexpr uint8_t resolution         = 8;
...
// Utility UI things
extern lv_obj_t* Utility;
extern bool Utility_tabview_flag;
extern bool Setting_tabview_flag;

// ===== DMA / buffer/sprite setup (only declarations here) =====
#define DMA 1

#if DMA
extern TFT_eSprite spr[2];   // sprites are defined in .cpp
extern bool        sprSel;   // toggle buffer select
extern lv_color_t* buf[2];   // pointers to sprite frame buffers
#else
extern lv_color_t  buf[2][/* line count */ 320 * 10]; // (kept as extern; sized in .cpp)
#endif

// General display constants (macros are fine in headers)
#define COLOR_DEPTH 16
#define IWIDTH  320
#define IHEIGHT 240

// Logging helper (declare only)
void my_print(const char *cstr);

// ISRs only need declarations in the header
void IRAM_ATTR VoltageEnc(void *arg);
void IRAM_ATTR CurrentEnc(void *arg);
void IRAM_ATTR ADCPinISR(void *arg);

// Keypad wiring (macros are fine)
#define I2CADDR 0x20
#define ROWS 6
#define COLS 6
